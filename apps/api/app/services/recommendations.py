...
SAFETY_NOTE = (
    ...
    "如出现胸闷胸痛、呼吸困难、昏厥、异常心悸、单侧小腿肿痛、持续疼痛或疼痛影响日常活动，请及时咨询医生或专业运动康复人员。"
)
...
def build_diet_advice(run_input: AnalyzeRecoveryRequest, high_load: bool) -> AdviceItem:
    if run_input.diet_preference == "fat_loss":
        content = (
            "跑后不要用极端节食抵消训练。安排一餐有主食、优质蛋白和蔬菜的恢复餐，份量保持清爽即可。"
            "如果距离正餐还有较久，可先用酸奶、鸡蛋、香蕉或少量面包垫一下，避免长时间空腹影响恢复。"
        )
    elif run_input.diet_preference == "vegetarian":
        content = (
            "跑后 1-2 小时内补充主食、豆制品或蛋奶类蛋白，再搭配蔬菜，帮助恢复能量和肌肉状态。"
            "如果当天跑量或 RPE 偏高，蛋白来源尽量不要只靠少量坚果，可增加豆腐、豆浆、鸡蛋或奶制品。"
        )
    elif run_input.diet_preference == "canteen":
        content = (
            "食堂场景可选米饭或面食、鸡蛋/鱼肉/豆腐和蔬菜，避免只喝饮料或只吃零食。"
            "如果胃口一般，先保证主食和蛋白质，再用清淡汤水或蔬菜补充一些水分和微量营养。"
        )
    elif run_input.diet_preference == "takeout":
        content = (
            "外卖优先选主食、蛋白质和蔬菜组合，少选重油重辣，避免影响睡眠和胃肠负担。"
            "可选盖饭、简餐、粥粉面加蛋白质这类稳定组合，不建议跑后只点甜饮或炸物。"
        )
    elif run_input.diet_preference == "light_night" or run_input.run_time_period == "night":
        content = (
            "夜跑后如果接近睡觉，选择温和易消化的小餐，如酸奶、香蕉、鸡蛋或少量主食。"
            "不要吃到过饱，也尽量减少酒精、浓茶和很辣的食物，避免影响入睡和胃肠恢复。"
        )
    else:
        content = (
            "跑后 1-2 小时内安排包含主食、蛋白质和蔬菜的恢复餐，不需要追求精确克数。"
            "如果这次训练时间较长或主观用力偏高，优先把碳水和蛋白质补齐，再考虑零食或补剂。"
        )

    if high_load:
        content += " 今天负荷偏高，不建议跑后长时间空腹；如果暂时吃不下正餐，先做小份补给，稍后再吃完整一餐。"
    elif run_input.user_level == "beginner":
        content += " 先把这一餐吃完整，比精确计算营养更重要，也能减少第二天疲劳感被低能量摄入放大。"
    elif run_input.user_level == "advanced":
        content += " 可把这餐视为补糖原和蛋白质摄入窗口，帮助后续训练恢复，并为下一次训练保留质量。"
...
def build_hydration_advice(run_input: AnalyzeRecoveryRequest, high_load: bool) -> AdviceItem:
    if high_load or run_input.duration_min >= 75:
        content = (
            "接下来 2-3 小时少量多次补水，不要一次性猛灌。"
            "如果出汗多、衣服有明显盐渍或跑后头胀乏力，可搭配正常餐食、淡盐汤或电解质饮品补充钠和水分。"
            "后续用口渴程度和尿色观察补水是否足够。"
        )
    else:
        content = (
            "跑后先少量补水，再根据口渴和尿色在接下来几小时继续补充，不需要一次喝太多。"
            "如果晚间跑步，睡前补水保持小口即可，避免喝太多影响睡眠。"
        )
...
def build_sleep_advice(run_input: AnalyzeRecoveryRequest, conservative: bool) -> AdviceItem:
    if run_input.run_time_period == "night":
        content = (
            "夜跑后减少屏幕刺激和高强度拉伸，给身体 20-30 分钟安静降温，尽量保证 7 小时以上睡眠。"
            "如果入睡前仍觉得兴奋，可以选择热水淋浴、呼吸放松或轻柔活动，不要再追加训练内容。"
        )
    elif run_input.sleep_hours < 6:
        content = (
            "昨晚睡眠不足，今晚优先提前入睡。恢复压力没有下降前，不建议用高强度训练硬顶。"
            "明早如果仍有明显困倦、腿沉或心率偏高，应主动降低训练目标。"
        )
    else:
        content = (
            "保持规律睡眠即可。若晚上仍感觉疲劳，优先提前休息而不是追加训练。"
            "睡前把补水和进食控制在舒适范围内，让身体有更稳定的恢复窗口。"
        )

    if conservative and run_input.sleep_hours < 7:
        content += " 睡眠是今天最值得补上的恢复资源，比继续增加训练量更能改善明日状态。"
    elif run_input.user_level == "advanced" and run_input.sleep_hours >= 7:
        content += " 若明日仍有训练，可用晨起精神、静息状态和腿部反馈再做负荷微调，必要时把质量段转为轻松有氧。"
...
def build_relaxation_advice(run_input: AnalyzeRecoveryRequest, high_load: bool) -> AdviceItem:
    if high_load or run_input.soreness_level >= 7:
        content = (
            "做 8-10 分钟慢走、呼吸放松或轻柔活动即可，避免用强刺激拉伸处理明显酸痛。"
            "如果局部疼痛尖锐、影响走路或逐渐加重，先停止拉伸和力量刺激，观察恢复情况。"
        )
    else:
        content = (
            "安排 8-10 分钟低强度拉伸、慢走或泡沫轴轻放松，让呼吸和心率逐步降下来。"
            "动作保持轻柔，不追求疼痛感；目标是恢复活动度，而不是把身体再刺激一遍。"
        )
...
def build_tomorrow_advice(
    ...
    if safety_flags:
        content += " 今天先记录症状出现时间、持续时间和变化趋势，明日不要用训练去测试不适是否消失。"
    elif score_result.score >= 61:
        content += " 执行时把身体反馈放在计划前面；如果热身后腿部沉重、酸痛升高或动作变形，就直接停止或改为恢复活动。"
    elif score_result.score >= 31:
        content += " 开始前用 10 分钟热身检查状态，过程中不追配速；只要疲劳感明显上升，就把训练降一级。"
    else:
        content += " 训练后继续观察疲劳和酸痛变化，把第二天的真实反馈作为下一次负荷调整依据。"
...
def build_timeline(
    ...
    if safety_flags:
        first_action = "停止追加训练，先补水、坐下休息，观察异常症状是否缓解；如果胸闷、头晕、呼吸困难、心悸或疼痛加重，不要继续运动。"
    else:
        first_action = "慢走冷身，少量补水，让呼吸和心率逐步降下来；不要立刻坐下很久，也不要马上追加冲刺或力量刺激。"

    meal_action = "安排包含主食、蛋白质和蔬菜的恢复餐。"
    if run_input.diet_preference == "fat_loss":
        meal_action = "安排清爽但完整的恢复餐，不用极端节食；先保证主食和蛋白质，再控制总量。"
    elif run_input.run_time_period == "night":
        meal_action = "如果接近睡前，选择温和易消化的小餐并少量补水，避免过饱、重辣或酒精影响入睡。"
    ...
    return [
        ...
        TimelineItem(time="跑后 15-45 分钟", action="换下湿衣物，继续少量多次补水，避免马上洗过热的澡；如果出汗很多，可随正餐补一点盐分。"),
        TimelineItem(time="跑后 1-2 小时", action=meal_action),
        TimelineItem(time="睡前", action="做轻柔放松，减少屏幕刺激，把睡眠作为今天的重点恢复动作；如果仍兴奋，优先呼吸放松而不是继续拉伸。"),
        TimelineItem(time="明早", action="起床后观察疲劳、酸痛、精神状态和走路感觉，再决定是否降级训练；不要只按原计划机械执行。"),
