...
5. 手动勾选 `异常心悸`、`单侧小腿痛/肿胀`、`尿色很深` 等异常信号，点击 `开始分析`，展示安全提示如何出现。

### 6.3 手动输入字段

| 字段 | 可选值或范围 | 说明 |
| --- | --- | --- |
| `距离 km` | `0.5` 到 `60`，最多 2 位小数 | 本次跑步距离。支持如 `8.25`，影响基础跑量负荷。 |
| `时长 min` | `5` 到 `360` | 本次跑步用时。前端用它计算配速，并辅助展示约几小时几分钟。 |
| `跑步类型` | 轻松跑、节奏跑、间歇跑、长距离跑、比赛 | 不同类型对应不同训练负荷。 |
| `间歇训练结构` | 组数、单组距离、组间休息、休息方式、强度备注 | 仅在跑步类型为间歇跑时展示，先用于前端确认摘要，不改变后端接口语义。 |
| `长距离补充` | 普通 LSD、目标配速段、后半程加速 | 仅在跑步类型为长距离跑时展示；目标配速段和后半程加速会同步到现有训练修饰项。 |
| `比赛补充` | 5km、10km、半马、全马、其他；是否接近全力 | 仅在跑步类型为比赛时展示；接近全力会同步到现有训练修饰项。 |
| `跑步时间` | 早晨、中午、傍晚、夜跑 | 夜跑会增加睡前恢复关注，RPE 高时影响更明显。 |
| `RPE 主观用力` | `1` 到 `10` | 用户对本次训练吃力程度的主观评分。 |
| `睡眠 h` | `0` 到 `14`，支持 0.5 小时等小数 | 昨晚睡眠时长。低睡眠会增加恢复压力。 |
| `疲劳` | `1` 到 `10` | 当前疲劳程度。 |
| `酸痛` | `1` 到 `10` | 腿部或身体酸痛程度。 |
| `平均心率` | 可为空，或 `40` 到 `230` | 可选字段；没有运动手表可以留空，系统会主要参考 RPE 和主观状态。 |
| `最大心率` | 可为空，或 `40` 到 `230` | 可选字段；用于辅助判断训练强度，不能低于平均心率。 |
...
| `异常信号` | 胸闷/胸痛、头晕、呼吸困难、异常心悸、昏厥/眼前发黑、恶心/呕吐、关节疼痛、疼痛影响走路、异常肿胀、麻木/刺痛、单侧小腿痛/肿胀、发热/感染感、尿色很深 | 触发安全提示、异常信号分项和更保守的明日建议。 |
...
| 分项贡献 | 展示跑量、类型、RPE、睡眠、疲劳、酸痛、心率、异常信号、时间、明日冲突等贡献。 |
...
| 场景 | 预期 |
| --- | --- |
| 距离输入 `8.25` | 字段无错误，确认页展示 `8.25 km`，可正常提交。 |
| 距离输入 `0.1` 或 `abc` | 字段下方显示原因，当前步骤不能继续或提交。 |
| 睡眠输入 `6.5` | 字段无错误，确认页展示 `6.5 h`。 |
| 平均心率 `160`、最大心率 `150` | 最大心率字段提示“最大心率不应低于平均心率”。 |
| 平均心率和最大心率留空 | 不报错，字段说明提示没有运动手表可以留空。 |
| 选择 `间歇跑` | 出现组数、单组距离、组间休息、休息方式和强度备注输入。 |
| 选择 `轻松跑` | 不展示间歇跑专项字段。 |
| 选择 `长距离跑` | 出现普通 LSD、目标配速段、后半程加速选项。 |
| 选择 `比赛` | 出现比赛距离和接近全力选项。 |
| RPE、疲劳、酸痛 | 桌面端和移动端都能清楚看到 1–10 评分锚点。 |
| 表单存在错误 | 不能开始分析；修正错误后可以正常分析。 |
| 使用演示案例、反馈、历史记录 | 原有 demo case、反馈提交、历史记录和模板兜底建议保持可用。 |
